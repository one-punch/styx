...
import com.xxshop.foundation.domain.GoodsTypeProperty;
...
import com.xxshop.foundation.domain.query.ReportQueryObject;
import com.xxshop.foundation.domain.query.TransportQueryObject;
...
import com.xxshop.foundation.service.IGoodsClassStapleService;
import com.xxshop.foundation.service.IGoodsService;
import com.xxshop.foundation.service.IGoodsSpecPropertyService;
import com.xxshop.foundation.service.IGoodsTypePropertyService;
...
import com.xxshop.foundation.service.IReportService;
import com.xxshop.foundation.service.IStoreService;
import com.xxshop.foundation.service.ISysConfigService;
import com.xxshop.foundation.service.ITransportService;
import com.xxshop.foundation.service.IUserConfigService;
import com.xxshop.foundation.service.IUserGoodsClassService;
...
import org.springframework.web.servlet.ModelAndView;
 ...
 public class GoodsSellerAction
 {
   ...
   @SecurityMapping(title="产品规格显示", value="/seller/goods_inventory.htm*", rtype="seller", rname="商品发布", rcode="goods_seller", rgroup="商品管理", display = false, rsequence = 0)
   ...
   public ModelAndView goods_inventory(HttpServletRequest request, HttpServletResponse response, String goods_spec_ids) {
     ...
     for (GoodsSpecification spec : specs) {
       ...
       for (GoodsSpecProperty gsp3 : gsps) {
   ...
   public static GoodsSpecProperty[][] list2group(List<List<GoodsSpecProperty>> list)
   {
     ...
     for (int i = 0; i < list.size(); i++) {
       gps[i] = ((GoodsSpecProperty[])((List)list.get(i)).toArray(
         new GoodsSpecProperty[((List)list.get(i)).size()]));
     }
     return gps;
   }
   ...
   public ModelAndView add_goods_finish(HttpServletRequest request, HttpServletResponse response, String id, String goods_class_id, String image_ids, String goods_main_img_id, String user_class_ids, String goods_brand_id, String goods_spec_ids, String goods_properties, String intentory_details, String goods_session, String transport_type, String transport_id)
   {
     ...
     if (goods_session1.equals("")) {
       ...
       GoodsClass gc = this.goodsClassService.getObjById(
         Long.valueOf(Long.parseLong(goods_class_id)));
       goods.setGc(gc);
       Accessory main_img = null;
       if ((goods_main_img_id != null) && (!goods_main_img_id.equals(""))) {
         main_img = this.accessoryService.getObjById(
           Long.valueOf(Long.parseLong(goods_main_img_id)));
       }
...
//				 for (UserGoodsClass localUserGoodsClass1 = 0; localUserGoodsClass1 < localUserGoodsClass2; localUserGoodsClass1++) {
         ...
         if (!ugc_id.equals("")) {
           UserGoodsClass ugc1 = this.userGoodsClassService
             .getObjById(Long.valueOf(Long.parseLong(ugc_id)));
           goods.getGoods_ugcs().add(ugc1);
         }
       }
...
//				 for (localUserGoodsClass2 = 0; localUserGoodsClass2 < localUserGoodsClass3; localUserGoodsClass2++) {
         ...
         if (!img_id.equals("")) {
           Accessory img2 = this.accessoryService.getObjById(
             Long.valueOf(Long.parseLong(img_id)));
           goods.getGoods_photos().add(img2);
         }
       }
...
//       for (Accessory img = 0; img < gsp; img++) {
         ...
         if (!property.equals("")) {
           ...
           map.put("name", this.goodsTypePropertyService
             .getObjById(Long.valueOf(Long.parseLong(list[0]))).getName());
           ((List)maps).add(map);
         }
       }
...
//				for (gsp = 0; gsp < localGoodsSpecProperty1; gsp++) {
       ...
       if (CommUtil.null2Int(transport_type) == 0) {
         Transport trans = this.transportService.getObjById(
           CommUtil.null2Long(transport_id));
   ...
   public void load_goods_class_staple(HttpServletRequest request, HttpServletResponse response, String id, String name) {
     GoodsClass obj = null;
     if ((id != null) && (!id.equals("")))
       obj = this.goodsclassstapleService.getObjById(Long.valueOf(Long.parseLong(id)))
         .getGc();
   ...
   public ModelAndView goods(HttpServletRequest request, HttpServletResponse response, String currentPage, String orderBy, String orderType, String goods_name, String user_class_id) {
     ...
     if ((user_class_id != null) && (!user_class_id.equals(""))) {
       UserGoodsClass ugc = this.userGoodsClassService.getObjById(
         Long.valueOf(Long.parseLong(user_class_id)));
   ...
   @SecurityMapping(title="商品编辑", value="/seller/goods_edit.htm*", rtype="seller", rname="商品编辑", rcode="goods_edit_seller", rgroup="商品管理", display = false, rsequence = 0)
   ...
   @SecurityMapping(title="商品上下架", value="/seller/goods_sale.htm*", rtype="seller", rname="商品上下架", rcode="goods_sale_seller", rgroup="商品管理", display = false, rsequence = 0)
   ...
   @SecurityMapping(title="商品删除", value="/seller/goods_del.htm*", rtype="seller", rname="商品删除", rcode="goods_del_seller", rgroup="商品管理", display = false, rsequence = 0)
   ...
   @SecurityMapping(title="举报图片查看", value="/seller/report_img.htm*", rtype="seller", rname="被举报禁售商品", rcode="goods_report_seller", rgroup="商品管理", display = false, rsequence = 0)
