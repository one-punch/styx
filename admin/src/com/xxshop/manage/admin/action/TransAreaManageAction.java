...
public class TransAreaManageAction {
	...
	@SecurityMapping(title = "运费地区列表", value = "/admin/trans_area_list.htm*", rtype = "admin", rname = "运费区域", rcode = "admin_trans_area", rgroup = "设置", display = false, rsequence = 0)
	...
	@SecurityMapping(title = "运费地区保存", value = "/admin/trans_area_save.htm*", rtype = "admin", rname = "运费区域", rcode = "admin_trans_area", rgroup = "设置", display = false, rsequence = 0)
	...
	public ModelAndView trans_area_save(HttpServletRequest request,
		...
		if (areaId != null) {
			...
			for (String id : ids) {
				...
				TransArea area = this.transareaService
						.getObjById(Long.valueOf(Long.parseLong(request
								.getParameter("id_" + i))));
		...
		if (!pid.equals(""))
			parent = this.transareaService.getObjById(Long.valueOf(Long
					.parseLong(pid)));
	...
	@SecurityMapping(title = "运费地区删除", value = "/admin/trans_area_del.htm*", rtype = "admin", rname = "运费区域", rcode = "admin_trans_area", rgroup = "设置", display = false, rsequence = 0)
	...
	public String trans_area_del(HttpServletRequest request, String mulitId,
		...
		for (String id : ids) {
			if (!id.equals("")) {
				Set list = genericIds(this.transareaService.getObjById(Long
	...
	@SecurityMapping(title = "运费地区Ajax更新", value = "/admin/trans_area_ajax.htm*", rtype = "admin", rname = "运费区域", rcode = "admin_trans_area", rgroup = "设置", display = false, rsequence = 0)
	@RequestMapping({ "/admin/trans_area_ajax.htm" })
	public void trans_area_ajax(HttpServletRequest request,
			HttpServletResponse response, String id, String fieldName,
			String value) throws ClassNotFoundException {
		TransArea obj = this.transareaService.getObjById(Long.valueOf(Long
				.parseLong(id)));
