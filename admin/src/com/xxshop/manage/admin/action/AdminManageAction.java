...
import com.xxshop.core.security.SecurityManager;
...
import com.xxshop.foundation.service.IResService;
import com.xxshop.foundation.service.IRoleGroupService;
...
import com.xxshop.manage.buyer.action.BaseBuyerAction;
import com.xxshop.manage.seller.action.BaseSellerAction;
import com.xxshop.view.web.action.CartViewAction;
...
public class AdminManageAction implements ServletContextAware {
	...
	@SecurityMapping(title = "管理员列表", value = "/admin/admin_list.htm*", rtype = "admin", rname = "管理员管理", rcode = "admin_manage", rgroup = "设置", display = false, rsequence = 0)
	...
	@SecurityMapping(title = "管理员添加", value = "/admin/admin_add.htm*", rtype = "admin", rname = "管理员管理", rcode = "admin_manage", rgroup = "设置", display = false, rsequence = 0)
	...
	@SecurityMapping(title = "管理员编辑", value = "/admin/admin_edit.htm*", rtype = "admin", rname = "管理员管理", rcode = "admin_manage", rgroup = "设置", display = false, rsequence = 0)
	...
	@SecurityMapping(title = "管理员保存", value = "/admin/admin_save.htm*", rtype = "admin", rname = "管理员管理", rcode = "admin_manage", rgroup = "设置", display = false, rsequence = 0)
	...
	public ModelAndView admin_save(HttpServletRequest request,
		...
		if (id.equals("")) {
			...
			User u = this.userService.getObjById(Long.valueOf(Long
					.parseLong(id)));
		...
		for (String rid : rids) {
			if (!rid.equals("")) {
				Role role = this.roleService.getObjById(Long.valueOf(Long
						.parseLong(rid)));
				user.getRoles().add(role);
			}
		}
		if (id.equals("")) {
			boolean sid = this.userService.save(user);
		} else {
	...
	@SecurityMapping(title = "管理员删除", value = "/admin/admin_del.htm*", rtype = "admin", rname = "管理员管理", rcode = "admin_manage", rgroup = "设置", display = false, rsequence = 0)
		...
		for (String id : ids) {
			if (!id.equals("")) {
				...
				if (!user.getUsername().equals("admin")) {
					this.databaseTools
							.execute("delete from xxshop_syslog where user_id="
									+ id);
					this.databaseTools
							.execute("delete from xxshop_user_role where user_id="
									+ id);
	...
	@SecurityMapping(title = "管理员修改密码", value = "/admin/admin_pws.htm*", rtype = "admin", rname = "商城后台管理", rcode = "admin_index", rgroup = "设置", display = false, rsequence = 0)
	...
	@SecurityMapping(title = "管理员密码保存", value = "/admin/admin_pws_save.htm*", rtype = "admin", rname = "商城后台管理", rcode = "admin_index", rgroup = "设置", display = false, rsequence = 0)
	...
	public String init_role() {
		...
		if ((current_user != null)
			...
			this.databaseTools.execute("delete from xxshop_role_res");
			this.databaseTools.execute("delete from xxshop_res");
			this.databaseTools.execute("delete from xxshop_user_role");
			this.databaseTools.execute("delete from xxshop_role");
			this.databaseTools.execute("delete from xxshop_rolegroup");
				...
				try {
					...
					for (Method m : ms) {
						...
						for (Annotation tag1 : annotation) {
							if (SecurityMapping.class.isAssignableFrom(tag1
									.annotationType())) {
								...
								if (ress.size() == 0) {
									Res res = new Res();
									res.setResName(((SecurityMapping) tag1)
											.title());
									res.setValue(value);
									res.setType("URL");
									res.setAddTime(new Date());
									this.resService.save(res);
									String rname = ((SecurityMapping) tag1)
											.rname();
									String roleCode = ((SecurityMapping) tag1)
											.rcode();
									...
									if (role == null) {
										role = new Role();
										role.setRoleName(((SecurityMapping) tag1)
												.rname());
									...
									role.setDisplay(((SecurityMapping) tag1)
											.display());
									role.setType(((SecurityMapping) tag1)
											.rtype().toUpperCase());

									String groupName = ((SecurityMapping) tag1)
											.rgroup();
