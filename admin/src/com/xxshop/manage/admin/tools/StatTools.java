...
public class StatTools {
	...
	public int query_all_goods() {
		List goods = this.goodsService.query("select obj from Goods obj", null,
				-1, -1);
		return goods.size();
	}

	public int query_all_store() {
		List stores = this.storeService.query("select obj from Store obj",
				null, -1, -1);
		return stores.size();
	}
	...
	public int query_update_store() {
		List stores = this.storeService
				.query("select obj from Store obj where obj.update_grade.id is not null",
						null, -1, -1);
		return stores.size();
	}
